...
void CadastroNotasSegundo(Aluno aluno[]){
	...
	if(aluno.matricula = matr){
		...
		aluno.media2 = (aluno.aep2 + aluno.prova2)/2;
	}else{
		std:: cout << "Digite uma matricula valida.";
		}
	}
...
void CadastraMediaFinal(Aluno aluno[]){
	...
	std:: cin >> matr;
	for(int i = 0; i < 1; i++){
		aluno.mediaFinal = ((aluno.media + aluno.media2)/2)
		if(aluno.mediaFinal < 6){
			std:: cout << "Digite a nota da substitutiva:\n";
			std:: cin >> aluno.sub;
			if(aluno.media >= aluno.media2){
				aluno.mediaFinal = ((aluno.aep2 + aluno.sub)/2);
			}else {
				aluno.mediaFinal = ((aluno.aep1 + aluno.sub)/2);
			}
		}
	}
}
